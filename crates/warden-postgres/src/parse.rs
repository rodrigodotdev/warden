//! Turning PostgreSQL text into a statement list, or into the one reason it could
//! not be.
...
/// sqlparser 0.62's own default and `warden-mysql`'s bound, so the two adapters
/// reject the same depth and an upstream default change cannot move either one
/// silently. It is the middle of three layers: `QueryRequest` caps the input at
/// 64 KiB before parsing (`docs/data-model.md` section 2), this bound caps nesting,
/// and sqlparser's default `recursive-protection` feature keeps a deep tree from
/// overflowing the stack (`docs/operations.md` section 2.4).
///
/// Measured against sqlparser 0.62 under `PostgreSqlDialect`: 2000 chained `OR`s
/// parse well within it, because an operator chain is iterative, not recursive.
/// What it stops is depth — 200 nested parentheses exceed it.
pub(crate) const RECURSION_LIMIT: usize = 50;

/// Why the PostgreSQL grammar produced no statement list.
///
...
/// Parses one PostgreSQL input into its statements.
///
...
mod tests {
    ...
    fn a_semicolon_inside_a_literal_or_a_comment_is_not_a_separator() {
        for sql in [
            ...
            "SELECT 1 /* ; */",
            "SELECT 1 -- ;",
            // Dollar quoting is PostgreSQL's other way to write a string, and a
            // scan that only understood single quotes would split this one.
            "SELECT $$a; b$$",
            "SELECT $tag$a; b$tag$",
        ] {
            assert_eq!(statements(sql).unwrap().len(), 1, "{sql}");
