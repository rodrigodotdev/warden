...
//!
//! Every failed tool call in `server.rs` leaves through `failure`, and so does
//! `output.rs`'s `ToolResponse::into_result` when a response cannot be serialized. No
//! other module builds a failed [`CallToolResult`].
