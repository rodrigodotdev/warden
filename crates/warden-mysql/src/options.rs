...
mod tests {
    ...
    #[test]
    fn no_production_call_in_this_crate_reaches_the_drivers_url_parser() {
        // Scans the whole crate, not just this file: a second module that parsed a
        // DSN for itself would let the query string decide TLS, the statement cache,
        // the socket and the character set again while every test here still passed.
        for (path, source) in crate_sources() {
            let calls = production_calls(&source);
            for forbidden in FORBIDDEN_CALLS {
                assert!(
                    !calls.iter().any(|call| call == forbidden),
                    "{path} calls {forbidden}; the DSN's query string would decide \
                     TLS, the statement cache, the socket and the character set \
                     again (ADR-0031)"
                );
            }
        }
