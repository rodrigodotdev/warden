...
//! this crate it does not — `src/connection.rs:187` carries a `#[cfg(test)] impl`
//! eighty-nine lines above the real test module — so the tail of that file was outside
//! every scan below, and no scan could have said so. There is no cutoff now:
...
use warden_guards::{
    Finding, TraitImpls, aliases_of, exported_names, macro_calls, method_calls, production_items,
    public_signature_types, source_files, string_literals, wildcard_arms,
};
...
///
/// These match whole path segments, so `MySqlConnectionPools` is not `MySqlConnection`
/// and `PoolSettings` is not `Pool`. That precision is why the list can name `Pool` at
/// all.
...
fn no_public_signature_names_a_parser_type() {
    ...
    // `impl QueryAnalyzer for MySqlAnalyzer`'s methods — the actual public contract
...
fn agent_sql_is_never_read_into_memory_before_it_is_bounded() {
    ...
    // `fetch_one` is correct there and wrong only for agent SQL. That one call is
    // exempted by its receiver, which names `CONNECTION_ID` in the very expression that
    // buffers; reading the receiver rather than the line is what makes the exemption
    // immune to how rustfmt happens to wrap the call.
    let path = file("execute.rs");
    let items = production_items(&path);
    for buffering in BUFFERING_FETCHES {
        for call in method_calls(&items, buffering) {
            assert!(
                call.detail.contains("CONNECTION_ID"),
...
fn the_only_format_in_execute_rs_builds_a_kill_query() {
    ...
    // real server, a *bound* `KILL QUERY ?` does not merely fail to decode — it does
    // not kill anything. `sqlx` 0.9.0's MySQL driver cannot decode the response,
    // confirmed against a container, not assumed: `Err(Protocol("unknown column type
    // 0xf3 ..."))`, `Com_kill` never increments, and the target query runs to
    // completion; the literal form is what actually kills. `kill` interpolates instead,
    // and that is safe only because `connection_id` is typed `u64` at the call site:
    // its formatted form is always `[0-9]+`, with no injection surface by
    // construction. The type is the guarantee; that the value is also server-sourced,
    // from Warden's own `SELECT CONNECTION_ID()`, only explains why it is correct, not
    // why it cannot be an injection. A second `format!` in this file needs its own
    // review rather than inheriting this exemption.
    //
    // Scoped to `src/execute.rs` only, deliberately not `src/container_tests/`: those
    // are test-only and `production_items` already removes them, but the scope is
    // stated because widening it would fire on legitimate fixture DDL — see
    // `wide_fixture`'s `format!("({id}, REPEAT('a', {payload_bytes}))")` — and the fix
    // for that false positive would be to loosen this guard, not to keep it.
    let formats = formats(&file("execute.rs"));
    for (line, literals) in &formats {
        assert!(
            literals.iter().any(|text| text.contains("KILL QUERY")),
            "src/execute.rs:{line} interpolates with format! outside the audited \
             KILL QUERY exception"
        );
    }
    assert!(
        !formats.is_empty(),
...
fn the_only_format_in_explain_rs_builds_a_kill_query() {
    ...
    // more: `KILL QUERY {connection_id}` with a `u64`, whose formatted form is always
    // `[0-9]+`. A second `format!` here needs its own review rather than the exemption
    // (`docs/operations.md` section 6.3).
    let formats = formats(&file("explain.rs"));
    for (line, literals) in &formats {
        assert!(
            literals.iter().any(|text| text.contains("KILL QUERY")),
            "src/explain.rs:{line} interpolates with format! outside the audited \
             KILL QUERY exception"
        );
    }
    assert!(
        !formats.is_empty(),
        "src/explain.rs no longer interpolates a KILL QUERY, so this guard passed on \
         an absence"
    );
}
